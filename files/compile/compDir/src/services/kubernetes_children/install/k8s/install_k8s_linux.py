"""
File in charge of containing the class that will install k8s for linux distributions.
"""
...
class InstallK8sLinux:
    ...
    def _manual_installation(self) -> int:
        ...
        status = self._download_file(self.k8s_link, self.k8s_file_name)
    ...
    def _install_for_aur(self) -> int:
        """ Install k8s for Aur systems """
        self.disp.sub_sub_title(
            "Installing k8s for Aur (Arch Linux User Repository) systems"
        )
        status = self.run(
            [
                "yay",
        ...
        self.disp.success_message("Installed k8s using yay ;-)")
        self.tty.current_tty_status = self.tty.success
        return status
    ...
    def _install_for_brew(self) -> int:
        ...
        self.disp.success_message("Installed k8s using brew ;-)")
        self.tty.current_tty_status = self.tty.success
        return self.tty.current_tty_status
